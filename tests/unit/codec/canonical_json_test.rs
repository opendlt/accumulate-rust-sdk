...
fn test_canonical_json_nested_objects() {
    ...
    // Debug: Print the actual canonical output
    println!("Canonical JSON output: {}", canonical);

    // Verify that keys are in alphabetical order at all levels
    assert!(canonical.starts_with(r#"{"metadata":"#), "Expected canonical JSON to start with metadata, but got: {}", canonical);
