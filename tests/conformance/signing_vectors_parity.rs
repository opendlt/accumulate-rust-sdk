use accumulate_client::crypto::ed25519::{Ed25519Signer, sha256, verify_signature, verify_signature_prehashed};
...
fn test_ed25519_deterministic_signing() {
    ...
    assert_eq!(signature1.to_bytes(), signature2.to_bytes(), "Signatures should be deterministic");

    // Test verification
    let pub_key_bytes = signer.public_key_bytes();
    let sig_bytes = signature1.to_bytes();

    assert!(
        verify_signature(&pub_key_bytes, message, &sig_bytes).is_ok(),
...
fn test_ed25519_different_seeds() {
    ...
    assert!(verify_signature(&signer1.public_key_bytes(), message, &sig1.to_bytes()).is_ok());
    assert!(verify_signature(&signer2.public_key_bytes(), message, &sig2.to_bytes()).is_ok());

    // Cross-verification should fail
    assert!(verify_signature(&signer1.public_key_bytes(), message, &sig2.to_bytes()).is_err());
    assert!(verify_signature(&signer2.public_key_bytes(), message, &sig1.to_bytes()).is_err());
}
...
fn test_ed25519_prehashed_signing() {
    ...
    let verification = verify_signature_prehashed(
        &signer.public_key_bytes(),
        &hash,
        &signature.to_bytes()
    );

    assert!(verification.is_ok(), "Prehashed signature should verify");

    // Also test that regular signing of the hash produces the same result
    let signature2 = signer.sign(&hash);
    assert_eq!(signature.to_bytes(), signature2.to_bytes(), "Prehashed and regular signing of hash should be identical");
}
...
fn test_ed25519_test_vectors() {
    ...
    for (seed, description) in &test_cases {
        ...
        assert!(
            verify_signature(&signer.public_key_bytes(), message, &signature.to_bytes()).is_ok(),
