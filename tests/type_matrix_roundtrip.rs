    ...
    match TransactionCodec::encode_envelope(&envelope) {
        Ok(encoded) => {
            ...
            match TransactionCodec::decode_envelope(&encoded) {
                    ...
                    match TransactionCodec::encode_envelope(&decoded) {
                        Ok(re_encoded) => {
                            if encoded == re_encoded {
                                ...
                                println!("  ❌ Binary roundtrip failed - re-encoded bytes differ");
                                println!("Original: {} bytes", encoded.len());
                                println!("Re-encoded: {} bytes", re_encoded.len());
                                println!("Original hex: {}", hex::encode(&encoded));
                                println!("Re-encoded hex: {}", hex::encode(&re_encoded));

                                // Show first few differences
                                for (i, (a, b)) in encoded.iter().zip(re_encoded.iter()).enumerate() {
                                    if a != b {
                                        println!("First difference at byte {}: original={:02x} re-encoded={:02x}", i, a, b);
                                        break;
                                    }
                                }
                ...
                Err(e) => {
                    println!("Failed to decode envelope - Error: {:?}", e);
                    println!("Encoded data length: {}", encoded.len());
                    println!("Encoded data: {}", hex::encode(&encoded));
                        ...
                        match TransactionCodec::encode_header(&decoded) {
                            Ok(re_encoded) => {
                                if encoded == re_encoded {
                                    ...
                                    println!("    ❌ Header {} binary roundtrip failed - re-encoded bytes differ", i);
                                    println!("    Original: {} bytes", encoded.len());
                                    println!("    Re-encoded: {} bytes", re_encoded.len());
                                    println!("    Original hex: {}", hex::encode(&encoded));
                                    println!("    Re-encoded hex: {}", hex::encode(&re_encoded));
                                    ...
                                    // Show first few differences
                                    for (j, (a, b)) in encoded.iter().zip(re_encoded.iter()).enumerate() {
                                        if a != b {
                                            println!("    First difference at byte {}: original={:02x} re-encoded={:02x}", j, a, b);
                                            break;
                                        }
                                    }
